...
class pos_config(osv.osv):
    ...
    def _get_group_pos_manager(self, cr, uid, context=None):
        group = self.pool.get('ir.model.data').get_object_reference(cr,uid,'backport_point_of_sale','group_pos_manager')
        if group:
            return group[1]
        else:
            return False
    ...
    def _get_group_pos_user(self, cr, uid, context=None):
        group = self.pool.get('ir.model.data').get_object_reference(cr,uid,'backport_point_of_sale','group_pos_user')
        if group:
            return group[1]
        else:
            return False
...
class pos_session(osv.osv):
    ...
    def wkf_action_close(self, cr, uid, ids, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            for st in record.statement_ids:
                if abs(st.difference) > st.journal_id.amount_authorized_diff:
                    ...
                    if not self.pool.get('ir.model.access').check_groups(cr, uid, "backport_point_of_sale.group_pos_manager"):
                        raise osv.except_osv( _('Error!'),
        ...
        obj = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'backport_point_of_sale', 'menu_point_root')[1]
        return {
